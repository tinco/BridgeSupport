...
class TestNullability < MiniTest::Test
  def setup
  ...
  def test_ret_type
    ...
    method4 = klass["method"][3]
    assert_equal(method4["selector"],          "return4")
    assert_equal(method4["retval"][0]["type"], "^{_TestStruct=i}")
  end
  ...
  def test_null_unspecified
    klass = @bs["class"][4]
    assert_equal(klass["name"], "EEE")
